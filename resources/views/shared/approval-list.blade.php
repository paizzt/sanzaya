...
<body class="bg-surface text-slate-800 font-sans antialiased overflow-hidden flex h-screen">
    ...
    <main class="flex-1 flex flex-col h-full w-full overflow-hidden bg-surface relative transition-all-ease">
        ...
        <header class="h-20 bg-white/80 backdrop-blur-md border-b border-slate-200/60 sticky top-0 z-30 flex items-center justify-between px-6 lg:px-10 transition-all-ease">
            ...
            <div class="flex items-center gap-4 nav-left">
                ...
                <div class="hidden md:block">
                    <h1 class="text-xl font-bold text-slate-800">Daftar Antrean SPPD</h1>
                </div>
            </div>
            ...
            <div class="flex items-center gap-4 lg:gap-6 nav-right">
                        ...
                        <div class="max-h-[320px] overflow-y-auto no-scrollbar notification-list">
                            @forelse($notifications as $notif)
                                ...
                                <a href="{{ route('approvals.show', $notif->id) }}" class="flex items-start gap-4 p-4 hover:bg-slate-50 border-b border-slate-50 transition-colors group notification-item">
                                    <div class="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 transition-colors {{ $notifClass }}">
                ...
                <div class="flex items-center gap-3 pl-4 border-l border-slate-200 user-profile">
                    <div class="hidden sm:block text-right user-info">
                        <p class="text-sm font-bold text-slate-800 leading-tight user-name">{{ Auth::user()->name ?? 'Nama User' }}</p>
        ...
        <div class="flex-1 overflow-y-auto p-6 lg:p-10 no-scrollbar content-area">
            ...
            @if(session('success'))
                <div class="mb-6 bg-emerald-50 border border-emerald-200 text-emerald-700 px-4 py-3 rounded-xl flex justify-between items-center shadow-sm">
            ...
            <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-soft mb-8">
                <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                    <div class="col-span-1 md:col-span-5">
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Cari Nama Pegawai</label>
                        <div class="relative">
                            <i class="fas fa-search absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"></i>
                            <input type="text" class="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 focus:border-brand-500 focus:ring focus:ring-brand-500/20 transition-all outline-none text-sm bg-slate-50 focus:bg-white" placeholder="Ketik nama pemohon...">
                        </div>
                    </div>
                    <div class="col-span-1 md:col-span-3">
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Filter Divisi</label>
            ...
            <div class="bg-white rounded-3xl border border-slate-200 shadow-soft overflow-hidden flex flex-col">
                ...
                <div class="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                    ...
                    <span class="text-xs font-semibold text-slate-500 bg-white px-3 py-1 border border-slate-200 rounded-full shadow-sm">{{ $requests->count() }} Dokumen</span>
                </div>
                    ...
                    <table class="w-full text-left border-collapse min-w-[800px]">
                        <thead>
                            <tr class="bg-white border-b border-slate-100 text-xs font-bold text-slate-400 uppercase tracking-wider">
                                <th class="p-5 w-1/4">Profil Pemohon</th>
                                <th class="p-5 w-1/4">Tujuan & Rute</th>
                                <th class="p-5 w-[15%]">Tgl Pengajuan</th>
                                <th class="p-5 w-[20%]">Status</th>
                                <th class="p-5 w-[15%] text-center">Tindakan</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-50">
                            @forelse($requests as $req)
                                <tr class="hover:bg-brand-50/50 transition-colors duration-200 group">
                                    ...
                                    <td class="p-5 align-middle">
                                        <p class="font-bold text-slate-800 text-sm mb-0.5 leading-tight">{{ \Carbon\Carbon::parse($req->created_at)->format('d M Y') }}</p>
                                        <p class="text-xs font-medium text-slate-500">{{ \Carbon\Carbon::parse($req->created_at)->format('H:i') }} WITA</p>
                                    </td>
                                    <td class="p-5 align-middle">
                                        @if($req->status == 'pending_l1')
                                            <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-amber-50 text-amber-600 border border-amber-200/60 shadow-sm">
                                                <i class="fas fa-hourglass-half"></i> Butuh ACC
                                            </span>
                                        @elseif($req->status == 'pending_l2')
                                            <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-indigo-50 text-indigo-600 border border-indigo-200/60 shadow-sm">
                                                <i class="fas fa-money-check"></i> Pencairan Dana
                                            </span>
                                        @endif
                                    </td>
                                    <td class="p-5 align-middle text-center">
                                        <a href="{{ route('approvals.show', $req->id) }}" class="inline-flex items-center gap-2 bg-white border border-slate-200 text-slate-700 hover:text-brand-600 hover:border-brand-300 hover:bg-brand-50 text-xs font-bold px-4 py-2 rounded-xl transition-all shadow-sm">
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="p-10 text-center">
                                        <div class="w-20 h-20 bg-emerald-50 rounded-full flex items-center justify-center text-emerald-400 text-3xl mx-auto mb-4">
                                            <i class="fas fa-clipboard-check"></i>
                                        </div>
                                        <h4 class="text-slate-700 font-bold mb-1 text-lg">Antrean Kosong</h4>
                                        <p class="text-sm text-slate-500 max-w-md mx-auto">Kerja bagus! Tidak ada pengajuan SPPD yang membutuhkan tindakan Anda saat ini.</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
