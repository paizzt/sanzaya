...
<body class="bg-surface text-slate-800 font-sans antialiased overflow-hidden flex h-screen">
    ...
    <aside id="sidebar" class="sidebar bg-white w-[280px] h-full border-r border-slate-200 flex flex-col transition-all-ease fixed lg:relative z-50 -translate-x-full lg:translate-x-0 shadow-2xl lg:shadow-none">
        ...
        <div class="flex-1 overflow-y-auto no-scrollbar py-6 px-4 space-y-1">
            ...
            <a href="{{ route('finance.settings') ?? '#' }}" class="menu-item flex items-center px-4 py-3 rounded-xl bg-brand-50 text-brand-600 font-medium transition-all-ease">
        ...
        <header class="h-20 bg-white/80 backdrop-blur-md border-b border-slate-200/60 sticky top-0 z-30 flex items-center justify-between px-6 lg:px-10 transition-all-ease">
            ...
            <div class="flex items-center gap-4 nav-left">
                ...
                <div class="hidden md:block">
                    <h1 class="text-xl font-bold text-slate-800">Pengaturan Akun Finance</h1>
                </div>
            </div>
                    ...
                    <div class="xl:col-span-2">
                        <div class="bg-white rounded-3xl border border-slate-200 shadow-soft p-6 sm:p-8 h-full">
                            <div class="flex items-center gap-3 mb-8 border-b border-slate-100 pb-4">
                                ...
                                <div>
                                    <h2 class="text-xl font-bold text-slate-800">Informasi Profil Finance</h2>
                                    <p class="text-slate-500 text-xs mt-0.5">Kelola identitas dan kontak akun Keuangan Anda.</p>
                                </div>
                            </div>
                            ...
                            <div class="flex flex-col sm:flex-row items-center sm:items-start gap-6 mb-8">
                                ...
                                <div class="text-center sm:text-left pt-2">
                                    <h5 class="text-lg font-bold text-slate-800 mb-1">{{ Auth::user()->name ?? 'Tim Keuangan' }}</h5>
                                    <p class="text-slate-500 text-sm mb-3">{{ Auth::user()->email ?? 'finance@sanzaya.com' }}</p>
                            ...
                            <form id="profileForm" action="{{ route('finance.settings.update') ?? '#' }}" method="POST">
                                @csrf
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                    ...
                                    <div>
                                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Divisi / Departemen</label>
                                        <input type="text" class="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-100 text-slate-500 font-medium outline-none cursor-not-allowed" value="{{ Auth::user()->division ?? 'Finance / Keuangan' }}" disabled>
                                        <p class="text-[10px] text-slate-400 mt-2 italic">Hubungi Admin Sistem jika ada perubahan divisi.</p>
                                    </div>
                                </div>
                        ...
                        <div class="bg-white rounded-3xl border border-slate-200 shadow-soft p-6 sm:p-8 h-full">
                            <div class="flex items-center gap-3 mb-6 border-b border-slate-100 pb-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-slate-500 text-xs mt-0.5">Ubah kata sandi akun Anda.</p>
                                </div>
                            </div>
                            
                            <p class="text-slate-500 text-xs mb-6 leading-relaxed">Pastikan Anda menggunakan kata sandi yang kuat dan rahasia untuk menjaga keamanan persetujuan keuangan Anda (L2).</p>

                            <form id="passwordForm" action="{{ route('finance.settings.update') ?? '#' }}" method="POST">
                                @csrf
                                <div class="mb-5">
