...
<body class="bg-surface text-slate-800 font-sans antialiased overflow-hidden flex h-screen">
    ...
    <!-- MENGAMBIL DATA STATISTIK DAN NOTIFIKASI FINANCE LANGSUNG -->
    @php
    ...
    <aside id="sidebar" class="sidebar bg-white w-[280px] h-full border-r border-slate-200 flex flex-col transition-all-ease fixed lg:relative z-50 -translate-x-full lg:translate-x-0 shadow-2xl lg:shadow-none">
        ...
        <div class="h-20 flex items-center justify-center border-b border-slate-100 px-6 logo-area overflow-hidden">
            <a href="{{ route('finance.dashboard') ?? '#' }}" class="flex items-center group">
        ...
        <div class="flex-1 overflow-y-auto no-scrollbar py-6 px-4 space-y-1">
            ...
            <a href="{{ route('finance.dashboard') ?? '#' }}" class="menu-item flex items-center px-4 py-3 rounded-xl bg-brand-50 text-brand-600 font-medium transition-all-ease">
            ...
            <a href="{{ route('approvals.index') ?? '#' }}" class="menu-item flex items-center px-4 py-3 rounded-xl text-slate-500 hover:bg-slate-50 hover:text-slate-900 font-medium transition-all-ease group">
                ...
                @if($menungguPencairan > 0)
                    <span class="ml-auto bg-brand-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full shadow-sm badge-count">{{ $menungguPencairan }}</span>
                @endif
            </a>
    ...
    <main class="flex-1 flex flex-col h-full w-full overflow-hidden bg-surface relative transition-all-ease">
        ...
        <header class="h-20 bg-white/80 backdrop-blur-md border-b border-slate-200/60 sticky top-0 z-30 flex items-center justify-between px-6 lg:px-10 transition-all-ease">
            ...
            <div class="flex items-center gap-4 nav-left">
                ...
                <div class="hidden md:block">
                    <h1 class="text-xl font-bold text-slate-800">Beranda Keuangan <span class="text-brand-500">(L2)</span></h1>
                </div>
            </div>
            ...
            <div class="flex items-center gap-4 lg:gap-6 nav-right">
                        ...
                        <div class="max-h-[320px] overflow-y-auto no-scrollbar notification-list">
                            @forelse($notifications as $notif)
                                <a href="{{ route('approvals.show', $notif->id) }}" class="flex items-start gap-4 p-4 hover:bg-slate-50 border-b border-slate-50 transition-colors group notification-item">
                                    <div class="w-10 h-10 rounded-full bg-emerald-50 text-emerald-500 flex items-center justify-center flex-shrink-0 group-hover:bg-emerald-100 transition-colors notification-icon">
                                        <i class="fas fa-file-invoice-dollar"></i>
                                    </div>
                                    <div class="flex-1 min-w-0 notification-content">
                                        <p class="text-sm text-slate-700 leading-snug mb-1">Pengajuan UC <strong>{{ $notif->user->name ?? 'Staff' }}</strong> ({{ $notif->destination }}) menunggu pencairan dana.</p>
                            ...
                            @empty
                                <div class="p-8 text-center notification-item">
                                    ...
                                    <p class="text-sm text-slate-500 font-medium notification-content">Tidak ada dokumen untuk dicairkan.</p>
                                </div>
                            @endforelse
                        </div>
                    </div>
                ...
                <div class="flex items-center gap-3 pl-4 border-l border-slate-200 user-profile">
                    <div class="hidden sm:block text-right user-info">
                        <p class="text-sm font-bold text-slate-800 leading-tight user-name">{{ Auth::user()->name ?? 'Finance' }}</p>
                        <p class="text-xs font-medium text-slate-500 capitalize user-role">{{ Auth::user()->role ?? 'finance' }}</p>
                    </div>
        ...
        <div class="flex-1 overflow-y-auto p-6 lg:p-10 no-scrollbar content-area">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
                <div>
                    <h2 class="text-2xl font-bold text-slate-800 mb-1">Halo, Tim {{ explode(' ', Auth::user()->name)[0] ?? 'Finance' }}! 👋</h2>
                    <p class="text-slate-500 text-sm">Berikut adalah ringkasan pencairan dana perjalanan dinas yang perlu diproses.</p>
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                ...
                <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-soft hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group stat-card">
                    <div class="flex justify-between items-start mb-4">
                        ...
                        @if($menungguPencairan > 0)
                            <span class="bg-brand-100 text-brand-700 text-xs font-bold px-2.5 py-1 rounded-lg">Action Needed</span>
                        @endif
                    </div>
                    <div class="stat-info">
                        <h3 class="text-3xl font-black text-slate-800 tracking-tight">{{ $menungguPencairan }}</h3>
                        <p class="text-sm font-semibold text-slate-500 uppercase tracking-wide mt-1">Butuh Pencairan</p>
                    </div>
                </div>
                ...
                <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-soft hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group stat-card">
                    ...
                    <div class="stat-info">
                        <h3 class="text-3xl font-black text-slate-800 tracking-tight">{{ $disetujuiFinance }}</h3>
                        <p class="text-sm font-semibold text-slate-500 uppercase tracking-wide mt-1">Telah Dicairkan</p>
                    </div>
                </div>
                ...
                <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-soft hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group stat-card">
                    ...
                    <div class="stat-info">
                        <h3 class="text-3xl font-black text-slate-800 tracking-tight">{{ $ditolakFinance }}</h3>
                        <p class="text-sm font-semibold text-slate-500 uppercase tracking-wide mt-1">Ditolak L2</p>
                    </div>
                </div>
                ...
                <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-soft hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group stat-card">
                    <div class="flex justify-between items-start mb-4">
                        <div class="w-12 h-12 rounded-xl bg-indigo-50 text-indigo-500 flex items-center justify-center text-xl group-hover:scale-110 transition-transform stat-icon">
                ...
                <div class="xl:col-span-1">
                    @if($menungguPencairan > 0)
                        <div class="bg-gradient-to-br from-brand-600 to-indigo-700 rounded-3xl p-8 text-white relative overflow-hidden shadow-lg shadow-brand-500/30 h-full flex flex-col justify-center action-card">
                            <!-- Decor Element -->
                            <i class="fas fa-money-check absolute -right-4 -bottom-6 text-[120px] text-white opacity-10 -rotate-12"></i>
                            ...
                            <div class="relative z-10">
                                ...
                                <h4 class="text-2xl font-bold mb-3 leading-tight">Ada {{ $menungguPencairan }} Pencairan Tertunda!</h4>
                                <p class="text-brand-50 text-sm mb-8 leading-relaxed">Dokumen SPPD ini telah lolos dari Manajer (L1) dan kini membutuhkan tindakan Anda untuk proses pencairan dana operasional.</p>
                                
                                <a href="{{ route('approvals.index') ?? '#' }}" class="inline-flex items-center justify-center gap-2 bg-white text-brand-600 font-bold px-6 py-3.5 rounded-xl hover:bg-brand-50 hover:scale-105 transition-all duration-300 shadow-md btn-light-custom">
                                    Tinjau & Cairkan <i class="fas fa-arrow-right"></i>
                                </a>
                            </div>
                        </div>
                    @else
                        <div class="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-3xl p-8 text-white relative overflow-hidden shadow-lg shadow-teal-500/30 h-full flex flex-col justify-center action-card">
                            ...
                            <div class="relative z-10">
                                ...
                                <p class="text-teal-50 text-sm mb-8 leading-relaxed">Kerja bagus! Tidak ada dokumen SPPD yang menunggu pencairan dana dari divisi Keuangan saat ini.</p>
                                
                                <a href="{{ route('finance.history') ?? '#' }}" class="inline-flex items-center justify-center gap-2 bg-white/20 border border-white/30 backdrop-blur-md text-white font-bold px-6 py-3.5 rounded-xl hover:bg-white/30 transition-all duration-300 btn-light-custom">
                                ...
                                <div class="flex items-center justify-between p-4 hover:bg-slate-50 rounded-2xl transition-colors duration-200 border border-transparent hover:border-slate-100 group list-item">
                                    ...
                                    <div class="flex items-center gap-4 item-left">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs font-medium text-slate-500 flex items-center gap-1.5 m-0">
                                                ...
                                                Mulai: {{ \Carbon\Carbon::parse($sppd->start_date)->format('d M Y') }}
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex flex-col items-end gap-2 text-end">
                                        ...
                                        <span class="text-[10px] font-semibold text-slate-400 bg-slate-100 px-2 py-0.5 rounded-md badge-status">
                                            Lolos L1: {{ \Carbon\Carbon::parse($sppd->updated_at)->diffForHumans() }}
                                        </span>
                                    </div>
                                </div>
                            @empty
                                <div class="h-full flex flex-col items-center justify-center p-10 text-center">
                                    ...
                                    <p class="text-sm text-slate-500 max-w-sm m-0">Tidak ada dokumen SPPD yang membutuhkan pencairan.</p>
                                </div>
                            @endforelse
                        </div>
