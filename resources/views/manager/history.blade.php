...
<head>
    ...
    <title>Riwayat Keputusan L1 - Satu Sanzaya</title>
...
<body class="bg-surface text-slate-800 font-sans antialiased overflow-hidden flex h-screen">
    ...
    <aside id="sidebar" class="sidebar bg-white w-[280px] h-full border-r border-slate-200 flex flex-col transition-all-ease fixed lg:relative z-50 -translate-x-full lg:translate-x-0 shadow-2xl lg:shadow-none">
        ...
        <div class="flex-1 overflow-y-auto no-scrollbar py-6 px-4 space-y-1">
            ...
            <a href="{{ route('manager.history') ?? '#' }}" class="menu-item flex items-center px-4 py-3 rounded-xl bg-brand-50 text-brand-600 font-medium transition-all-ease">
        ...
        <header class="h-20 bg-white/80 backdrop-blur-md border-b border-slate-200/60 sticky top-0 z-30 flex items-center justify-between px-6 lg:px-10 transition-all-ease">
            ...
            <div class="flex items-center gap-4 nav-left">
                ...
                <div class="hidden md:block">
                    <h1 class="text-xl font-bold text-slate-800">Riwayat Keputusan SPPD</h1>
                </div>
            </div>
                        ...
                        <div class="max-h-[320px] overflow-y-auto no-scrollbar notification-list">
                            @forelse($notifications as $notif)
                                <a href="{{ route('approvals.show', $notif->id) }}" class="flex items-start gap-4 p-4 hover:bg-slate-50 border-b border-slate-50 transition-colors group notification-item">
                                    <div class="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 transition-colors bg-amber-50 text-amber-500 group-hover:bg-amber-100">
                                        <i class="fas fa-file-signature"></i>
                                    </div>
                                    <div class="flex-1 min-w-0 notification-content">
                                        <p class="text-sm text-slate-700 leading-snug mb-1">Pengajuan UC dari <span class='font-bold text-slate-900'>{{ $notif->user->name ?? 'Staff' }}</span> menunggu persetujuan Anda.</p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                    <div class="col-span-1 md:col-span-5">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" class="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 focus:border-brand-500 focus:ring focus:ring-brand-500/20 transition-all outline-none text-sm bg-slate-50 focus:bg-white" placeholder="Ketik kata kunci...">
                        </div>
                    </div>
            ...
            <div class="bg-white rounded-3xl border border-slate-200 shadow-soft overflow-hidden flex flex-col">
                ...
                <div class="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                    <h3 class="font-bold text-slate-800 flex items-center gap-2 m-0 text-base">
                        ...
                        Daftar Dokumen yang Pernah Anda Proses (L1)
                    </h3>
                </div>
                    ...
                    <table class="w-full text-left border-collapse min-w-[900px]">
                        <thead>
                            <tr class="bg-white border-b border-slate-100 text-xs font-bold text-slate-400 uppercase tracking-wider">
                                ...
                                <th class="p-5 w-1/4">Rute & Pelaksanaan</th>
                                <th class="p-5 w-[15%]">Status Akhir</th>
                                <th class="p-5 w-[25%]">Catatan Anda</th>
                        ...
                        <tbody class="divide-y divide-slate-50">
                            @forelse($requests as $req)
                                <tr class="hover:bg-brand-50/50 transition-colors duration-200 group">
                                    ...
                                    <td class="p-5 align-middle">
                                        @if($req->status == 'pending_l2')
                                            <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-indigo-50 text-indigo-600 border border-indigo-200/60 shadow-sm">
                                                <i class="fas fa-money-check"></i> Lolos L1 (Cek Finance)
                                            </span>
                                        ...
                                        @elseif($req->status == 'rejected')
                                            <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-rose-50 text-rose-600 border border-rose-200/60 shadow-sm">
                                                <i class="fas fa-times-circle"></i> Anda Tolak
                                            </span>
                                        @else
                                    ...
                                    <td class="p-5 align-middle">
                                        @if($req->l1_note)
                                            <p class="text-xs text-slate-600 italic leading-relaxed m-0 line-clamp-2">"{{ $req->l1_note }}"</p>
                                        @else
                                            <span class="text-xs text-slate-400 italic">Tanpa catatan.</span>
                                        @endif
                                    </td>
                                    <td class="p-5 align-middle text-center">
                                        <a href="{{ route('approvals.show', $req->id) }}" class="inline-flex items-center justify-center w-8 h-8 bg-white border border-slate-200 text-brand-600 hover:text-white hover:border-brand-600 hover:bg-brand-600 rounded-lg transition-all shadow-sm" title="Lihat Detail">
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="p-10 text-center">
                                        ...
                                        <p class="text-sm text-slate-500 max-w-md mx-auto">Anda belum melakukan tindakan (ACC/Tolak) pada pengajuan SPPD manapun.</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
