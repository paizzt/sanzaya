...
<head>
    ...
    <style>
        ...
        .sidebar.collapsed .company-name { display: none; }
        ...
        /* Saat sidebar PC mengecil, konten utama melebar */
        ...
        .user-role { font-size: 11px; color: var(--text-gray); margin: 0; }
        ...
        @media (max-width: 992px) {
            .search-bar { width: 200px; } /* Mengecilkan kolom search di layar tablet */
        }

        @media (max-width: 768px) {
            /* Sidebar menjadi melayang (off-canvas) dan sembunyi di kiri */
            .sidebar {
                position: fixed; left: -100%;
                box-shadow: 4px 0 15px rgba(0,0,0,0.1);
            }
            
            /* Class baru saat sidebar dipanggil di HP */
            .sidebar.mobile-active { left: 0; }
            .sidebar-overlay.active { display: block; }
            
            /* Konten Utama mengambil 100% layar HP */
            .main-content { width: 100%; }
            .sidebar.collapsed ~ .main-content { width: 100%; }

            /* Sembunyikan search bar agar header tidak tabrakan di HP */
            .search-bar { display: none; }
            
            .top-navbar { padding: 0 20px; }
            .content-area { padding: 20px; }
            
            /* Sembunyikan Role agar teks profil pas */
...
<body>
    ...
    <script>
        ...
        toggleBtn.addEventListener('click', function() {
            // Jika layar ukuran HP (< 768px), sidebar muncul sebagai pop-up
            if (window.innerWidth <= 768) {
                sidebar.classList.toggle('mobile-active');
                overlay.classList.toggle('active');
            } else {
                // Jika layar PC, sidebar mengecil
                sidebar.classList.toggle('collapsed');
            }
        });

        // Menutup sidebar saat layar gelap (overlay) diklik di HP
        overlay.addEventListener('click', function() {
            sidebar.classList.remove('mobile-active');
            overlay.classList.remove('active');
        });

        // Reset tampilan jika ukuran layar browser ditarik/diubah
